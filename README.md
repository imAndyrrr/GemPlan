...
- **多账号管理与智能配额池**：
  ...
  - **智能配额路由与优先级分层**：
    - 支持为每个账号手动设置优先级（0-100 整数，数值越大越优先）。
    - 调度时先消耗健康高优先级账号；同优先级账号内按实时模型配额百分比打分，配额相同再按 LRU 均衡轮询。
    - **无缝自动故障转移 (Failover)**：当高优先级账号遭遇 429 速率限制、临时冷却或鉴权错误时，在同一次请求内自动顺延降级至下一优先级/可用账号，用户侧零感知报错。
  - **可视化账号与配额池控制台**：直观查看全部账号状态、各模型聚合配额与单账号配额，支持在界面直接调整优先级、一键启用/禁用、删除和解除冷却。
- **可视化 Web 管理控制台**：
